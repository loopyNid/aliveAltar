...
#define MAX_DISTANCE 480 // Maximum distance we want to ping for (in centimeters). Maximum sensor distance is rated at 400-500cm.
#define trigPin2 5
#define echoPin2 4
#define trigPin3 11
#define echoPin3 10
...
//  Serial.println(brightness);
  ...
  analogWrite(ledW, 255 - brightness);
  ...
  Serial.println(uS1RNDT); // Send ping, get distance in cm and print result (0 = outside set distance range)
    ...
    if(((uS1RNDT < ledChange) && (uS1RNDT > 2)) || ((uS2RNDT < ledChange) && (uS2RNDT > 2))||((uS3RNDT < ledChange) && (uS3RNDT > 2)) || ((uS4RNDT < ledChange) && (uS4RNDT > 2))){
//      Serial.println('+');
       if(brightness >= 245){
         brightness = 255;
       } else {
    ...
    }else if((uS1RNDT >= ledChange)|| (uS2RNDT >= ledChange)|| (uS3RNDT >= ledChange) || (uS4RNDT >= ledChange)){
//      Serial.println('-');
       if( brightness <= 10 ){
         brightness = 0;
       } else {
